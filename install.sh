...
read -rp "Schedule? minutes (e.g. 295) or times (e.g. 9,14,19:30) [295]: " in
in=${in:-295}
...
if [[ "$in" == *[,:]* ]]; then
  for p in ${in//,/ }; do
    h=${p%%:*}; m=${p##*:}; [[ "$m" == "$h" ]] && m=0
    [[ "$h$m" =~ ^[0-9]+$ ]] && (( h<24 && m<60 )) || { echo "✗ bad time: $p"; exit 1; }
  ...
  [[ "$in" =~ ^[1-9][0-9]*$ ]] || { echo "✗ bad input: $in"; exit 1; }
  INTERVAL=$(( in * 60 ))
...
<plist version="1.0"><dict>
  ...
  <key>EnvironmentVariables</key><dict><key>INTERVAL</key><string>$INTERVAL</string><key>CLAUDE_BIN</key><string>$CLAUDE</string></dict>
